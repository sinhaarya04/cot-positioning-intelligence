    ...
    for market in MARKET_CONFIG.keys():
        ...
        if MARKET_CONFIG[market]["eia_series"] is not None:
            ...
            if not mkt_eia.empty:
                ...
                # Merge via asof join (nearest date match)
                cot_dates = mkt_legacy[["report_date"]].copy().sort_values("report_date")
                eia_dates = mkt_eia[["report_date", "inv_z"]].copy().sort_values("report_date")
                ...
                z_scores_arr = pd.Series(z_score_52w, index=mkt_legacy.index)
                inv_z_arr = merged["inv_z"].values
                
                for i, idx in enumerate(mkt_legacy.index):
                    if pd.notna(inv_z_arr[i]):
                        inv_mismatch.at[idx] = float(inv_z_arr[i]) * float(z_scores_arr.iloc[i])
